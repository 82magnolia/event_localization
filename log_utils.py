...
def visualize_img(img, log_dir, img_prefix, fig_no):
    assert len(img.shape) == 2 or len(img.shape) == 3
    if len(img.shape) == 3:
        if img.shape[0] == 1 or img.shape[0] == 3:  # Pytorch Tensor case
            vis_img = img.permute(1, 2, 0).numpy()
        else:
            vis_img = img

        plt.imshow(vis_img)
        plt.savefig(os.path.join(log_dir, f'{img_prefix}_{fig_no}.png'))
    else:
        plt.imshow(np.stack([img] * 3, axis=-1))
        plt.savefig(os.path.join(log_dir, f'{img_prefix}_{fig_no}.png'))


def save_img(img, log_dir, img_filename):
    if len(img.shape) == 3:
        if img.shape[0] == 1 or img.shape[0] == 3:  # Pytorch Tensor case
            vis_img = img.permute(1, 2, 0).numpy()
        else:
    ...
    cv2.imwrite(os.path.join(log_dir, img_filename), vis_img)
